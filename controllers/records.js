...
function _getQuery(searchType, limit, offset, searchValue, postcode) {
  ...
  switch (searchType) {
    case "name":
      ...
      queryStatement = `SELECT * FROM ${recordsTable} WHERE name ='${searchValue}' ORDER BY ic asc OFFSET ${offset} ROWS FETCH NEXT ${limit} ROWS ONLY`;
      break;
    case "address":
      searchValue = searchValue.replaceAll(" ", "%");
      queryStatement = `SELECT * FROM ${recordsTable} WHERE address LIKE '%${searchValue}%' AND postcode = '${
        postcode == undefined ? "" : postcode
      }' ORDER BY ic asc OFFSET ${offset} ROWS FETCH NEXT ${limit} ROWS ONLY`;
      break;
    case "phone":
      queryStatement = `SELECT * FROM ${recordsTable} WHERE tel1 = '${searchValue}' OR tel2 = '${searchValue}' OR tel3 = '${searchValue}' ORDER BY ic asc OFFSET ${offset} ROWS FETCH NEXT ${limit} ROWS ONLY`;
      break;

    case "ic":
      queryStatement = `SELECT * FROM ${recordsTable} WHERE ic = '${searchValue}' ORDER BY ic asc OFFSET ${offset} ROWS FETCH NEXT ${limit} ROWS ONLY`;
      break;
